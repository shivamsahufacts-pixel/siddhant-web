...
export default function PortfolioSection() {
  ...
  // Calculate project card width including margin/gap
  const getCardWidth = () => {
    if (!projectCardRef.current) return 0;
  ...
  const scrollTo = (index) => {
    if (!scrollContainerRef.current || index < 0 || index >= projects.length) return;
  ...
  const handleScroll = () => {
    ...
    // Calculate current active index with rounding
    const newActive = Math.round(scrollLeft / cardWidth);

    if (newActive !== active && newActive >= 0 && newActive < projects.length) {
      setActive(newActive);
    }
  };
  ...
  return (
    <section className="relative py-10 bg-[var(--bg)] text-[var(--text-color)] overflow-hidden">
      {/* Glow effects */}
      <div className="absolute -top-20 -left-20 w-96 h-96 bg-[var(--accent)] opacity-10 blur-3xl rounded-full" />
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-purple-500 opacity-10 blur-3xl rounded-full" />
      ...
      <div className="relative max-w-6xl mx-auto px-4">
        ...
        {projects.length > 1 && (
          <>
            <button
              onClick={scrollPrev}
              disabled={active === 0}
              className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 p-3 rounded-full bg-[var(--card-bg)] border border-[var(--border)] shadow-lg hover:bg-[var(--accent)] hover:text-white transition-all duration-300 disabled:opacity-30 disabled:cursor-not-allowed"
            ...
            <button
              onClick={scrollNext}
              disabled={active === projects.length - 1}
              className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 p-3 rounded-full bg-[var(--card-bg)] border border-[var(--border)] shadow-lg hover:bg-[var(--accent)] hover:text-white transition-all duration-300 disabled:opacity-30 disabled:cursor-not-allowed"
        ...
        <div
          ref={scrollContainerRef}
          onScroll={handleScroll}
          className="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-6 scroll-smooth hide-scrollbar"
          style={{
            scrollbarWidth: 'none',
            msOverflowStyle: 'none'
          }}
        >
          ...
          {projects.map((p, i) => (
            <div
              key={p._id}
              ref={i === 0 ? projectCardRef : null}
              className="min-w-[85%] sm:min-w-[70%] md:min-w-[380px] snap-center"
            >
              <div className="bg-[var(--card-bg)] rounded-3xl border border-[var(--border)] overflow-hidden hover:scale-[1.02] transition-all duration-300 shadow-lg">
                {/* Image */}
                <Link href={`/projects/${p.slug}`}>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {p.tech.length > 5 && (
                      <span className="text-xs px-3 py-1 rounded-full bg-gray-700/50 text-white">
                  ...
                  <div className="flex flex-wrap gap-3 pt-4">
                    {p.live !== "#" && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 px-4 py-2 text-sm rounded-lg bg-[var(--accent)] text-white hover:scale-105 transition-transform"
                      >
                    ...
                    {p.github !== "#" && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 px-4 py-2 text-sm rounded-lg border border-[var(--border)] hover:border-[var(--accent)] transition-colors"
                      >
                    ...
                    <Link
                      href={`/projects/${p.slug}`}
                      className="flex items-center gap-2 px-4 py-2 text-sm rounded-lg border border-[var(--border)] hover:border-[var(--accent)] hover:bg-[var(--accent)]/10 transition-colors"
                    >
                      View Details →
                    </Link>
                  </div>
            ...
            {projects.map((_, i) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                  active === i
                    ? "bg-[var(--accent)] scale-125"
                    : "bg-gray-500/40 hover:bg-gray-500/70 hover:scale-110"
                }`}
                aria-label={`Go to project ${i + 1}`}
              />
            ))}
      ...
      {projects.length === 0 && !loading && (
        <div className="text-center py-12">
          <div className="inline-block p-8 rounded-full bg-gray-800/30 mb-4">
            <FaGithub className="text-4xl text-gray-500" />
          </div>
          <h3 className="text-2xl font-bold mb-2">No Projects Yet</h3>
          <p className="text-gray-400">
